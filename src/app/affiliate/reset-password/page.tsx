...
export default function AffiliateResetPasswordPage() {
  ...
  const validateToken = async () => {
    try {
      const response = await fetch(`/api/affiliates/auth/reset-password?token=${token}`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('/api/affiliates/auth/reset-password', {
        method: 'POST',
        headers: {
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-50 p-4">
      <Card className="w-full max-w-md">
        <CardHeader className="text-center pb-4">
          ...
          <CardDescription className="text-sm">
            {affiliate ? `Reset password for ${affiliate.name} (${affiliate.email})` : 'Enter your new password'}
          </CardDescription>
        </CardHeader>
